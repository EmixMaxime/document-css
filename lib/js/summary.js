class Summary {
    ...
    constructor(containerThings, navigationContainer, titles) {
        let container
        if (containerThings instanceof Node) {
            container = containerThings
        } else {
            container = document.querySelector(containerThings)
        }

        this.titles = titles || Array.from(container.querySelectorAll('h2, h3, h4, h5'))
    ...
    getLevelOfTitle(title) {
        return parseInt(title.tagName.replace('H', ''), 10) - 1
    }
    ...
    addSectitonHeader() {
        const sections = Array.from(document.querySelectorAll('#to-summarize section.sheet'))
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            const previousH2 = this.getPreviousTag(previousSections, 'h2')
            const previousH3 = this.getPreviousTag(previousSections, 'h3')
            ...
            if (previousH3 && !section.querySelector('h3')) {
    ...
    addSectionFooter() {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            footer.textContent = "IUT du Littoral Côte d'Opale Calais 2017 - 2018"
